...
class TestCrossovers(unittest.TestCase):
    ...
    def test_one_point(self, random):
        random.randint.return_value = 3
        c, d = crossover('one_point')((self.a, self.b))
        random.randint.assert_called_once_with(0, self.a.genes_count)
        self.assertEqual([0, 1, 2, 13, 14, 15, 16], c)
        self.assertEqual([10, 11, 12, 3, 4, 5, 6], d)
    
    @patch('Crossovers.random')
    def test_two_point(self, random):
        random.randint.side_effect = [2, 5]
        c, d = crossover('two_point')((self.a, self.b))
        actual_calls = random.randint.call_args_list
    ...
    @patch('Crossovers.random')
    def test_anular_straight(self, random):
        random.randint.side_effect = [3, 2]
    ...
    @patch('Crossovers.random')
    def test_anular_round(self, random):
        random.randint.side_effect = [5, 3]
        c, d = crossover('anular')((self.a, self.b))
        actual_calls = random.randint.call_args_list
        expected_calls = [call(0, self.a.genes_count - 1), call(1, self.a.genes_count // 2)]
        self.assertEqual(actual_calls, expected_calls)
    ...
    def test_uniform(self, random):
        random.random.side_effect = [0.1, 0.9, 0.1, 0.1, 0.9, 0.9, 0.1]
        c, d = crossover('uniform')((self.a, self.b))
        self.assertEqual(self.a.genes_count, random.random.call_count)
